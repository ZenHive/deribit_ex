defmodule DeribitEx.RPCTest do
  ...
  describe "generate_request/3" do
    test "generates proper JSON-RPC request structure" do
      ...
      {:ok, payload, id} = RPC.generate_request(method, params)
    ...
    test "uses provided ID when specified" do
      ...
      {:ok, payload, returned_id} = RPC.generate_request(method, params, id)
    ...
    test "handles complex parameter structures" do
      ...
      {:ok, payload, _id} = RPC.generate_request(method, params)
    ...
    test "emits telemetry events when generating request" do
      ...
      {:ok, _payload, id} = RPC.generate_request(method, params)
    ...
    test "raises FunctionClauseError when params is not a map" do
      ...
      assert_raise FunctionClauseError, fn ->
        RPC.generate_request(method, invalid_params)
      end

      assert_raise FunctionClauseError, fn ->
        RPC.generate_request(method, ["array", "not", "a", "map"])
      end

      assert_raise FunctionClauseError, fn ->
        RPC.generate_request(method, 123)
      end
    end
  end
  ...
  describe "parse_response/1" do
    test "handles successful response with result" do
      ...
      assert RPC.parse_response(response) == {:ok, "success"}
    end
    ...
    test "handles error response" do
      ...
      assert RPC.parse_response(response) == {:error, error}
    end
    ...
    test "handles invalid response structure and emits telemetry" do
      ...
      assert RPC.parse_response(invalid_response) ==
  ...
  describe "needs_reauth?/1" do
    test "detects authentication required errors" do
      # not_authorized
      assert RPC.needs_reauth?(%{"code" => 13_004, "message" => "Not authorized"})
      # invalid_token
      assert RPC.needs_reauth?(%{"code" => 13_009, "message" => "Invalid token"})
      # token_expired
      assert RPC.needs_reauth?(%{"code" => 13_010, "message" => "Token expired"})
      # token_missing
      assert RPC.needs_reauth?(%{"code" => 13_011, "message" => "Access token required"})
    end

    test "returns false for other error types" do
      refute RPC.needs_reauth?(%{"code" => 10_001, "message" => "Error"})
      refute RPC.needs_reauth?(%{"message" => "No code"})
      refute RPC.needs_reauth?("Not a map")
    end
  end

  describe "classify_error/1" do
    test "classifies authentication errors" do
      assert RPC.classify_error(%{"code" => 13_004, "message" => "Not authorized"}) ==
               {:auth, :not_authorized, "Not authorized"}

      assert RPC.classify_error(%{"code" => 13_009, "message" => "Invalid token"}) ==
               {:auth, :invalid_token, "Invalid token"}

      assert RPC.classify_error(%{"code" => 13_010, "message" => "Token expired"}) ==
               {:auth, :token_expired, "Token expired"}
    end

    test "classifies rate limit errors" do
      assert RPC.classify_error(%{"code" => 10_429, "message" => "Too many requests"}) ==
               {:rate_limit, :too_many_requests, "Too many requests"}

      assert RPC.classify_error(%{"code" => 11_010, "message" => "Max requests exceeded"}) ==
               {:rate_limit, :max_requests_exceeded, "Max requests exceeded"}
    end

    test "classifies validation errors" do
      assert RPC.classify_error(%{"code" => 10_001, "message" => "Invalid params"}) ==
               {:validation, :invalid_params, "Invalid params"}
    end

    test "handles unknown error codes" do
      assert RPC.classify_error(%{"code" => 99_999, "message" => "Unknown error"}) ==
               {:unknown, :unknown_code, "Unknown error"}
    end

    test "handles invalid error format" do
      assert RPC.classify_error("not a map") ==
               {:unknown, :invalid_error_format, "Invalid error format"}

      assert RPC.classify_error(%{}) ==
               {:unknown, :invalid_error_format, "Invalid error format"}
    end
  end
  ...
  describe "method_type/1" do
    test "correctly identifies public methods" do
      ...
      assert RPC.method_type("public/test") == :public
    end

    test "correctly identifies private methods" do
      assert RPC.method_type("private/get_position") == :private
      assert RPC.method_type("private/enable_cancel_on_disconnect") == :private
      assert RPC.method_type("private/get_transfers") == :private
    end

    test "returns :unknown for unrecognized methods" do
      assert RPC.method_type("get_time") == :unknown
      assert RPC.method_type("invalid") == :unknown
      assert RPC.method_type("") == :unknown
    end
  end

  describe "requires_auth?/1" do
    test "returns true for private methods" do
      assert RPC.requires_auth?("private/get_position")
      assert RPC.requires_auth?("private/enable_cancel_on_disconnect")
    end

    test "returns false for public methods" do
      refute RPC.requires_auth?("public/get_time")
      refute RPC.requires_auth?("public/auth")
    end

    test "returns false for unrecognized methods" do
      refute RPC.requires_auth?("invalid_method")
      refute RPC.requires_auth?("")
    end
  end
  ...
  describe "add_auth_params/3" do
    test "adds access_token for private methods" do
      ...
      result = RPC.add_auth_params(params, "private/get_position", token)
    ...
    test "does not add token for public methods" do
      ...
      result = RPC.add_auth_params(params, "public/get_time", token)
    ...
    test "does not modify params when token is nil" do
      ...
      result = RPC.add_auth_params(params, "private/get_position", nil)
  ...
  describe "track_request/5" do
    test "adds request to state" do
      ...
      updated_state = RPC.track_request(state, id, method, params)
    ...
    test "uses method-specific timeout values" do
      ...
      auth_state = RPC.track_request(state, 1, "public/auth", %{})

      # Test method should use 2_000 ms timeout
      test_state = RPC.track_request(state, 2, "public/test", %{})
    ...
    test "respects override timeout value in options" do
      ...
      updated_state = RPC.track_request(state, 1, "public/get_time", %{}, options)
  ...
  describe "remove_tracked_request/2" do
    test "removes specified request from state" do
      ...
      updated_state = RPC.remove_tracked_request(state, 12_345)
    ...
    test "handles non-existent request IDs gracefully" do
      ...
      updated_state = RPC.remove_tracked_request(state, 9999)
    ...
    test "preserves other requests" do
      ...
      updated_state = RPC.remove_tracked_request(state, 1111)
    ...
    test "handles missing or invalid timeout_ref" do
      ...
      updated_state1 = RPC.remove_tracked_request(state, 1111)
      updated_state2 = RPC.remove_tracked_request(updated_state1, 2222)
  ...
  describe "error/2" do
    test "creates standardized error structure" do
      {:error, error} = RPC.error(:connection_failed)
    ...
    test "includes details when provided" do
      ...
      {:error, error} = RPC.error(:connection_failed, details)
  ...
  describe "extract_metadata/1" do
    test "extracts metadata from subscription notifications" do
      ...
      metadata = RPC.extract_metadata(notification)
    ...
    test "extracts timing information from responses" do
      ...
      metadata = RPC.extract_metadata(response)
  ...
  describe "microseconds_to_datetime/1" do
    test "converts microsecond timestamps to DateTime" do
      ...
      datetime = RPC.microseconds_to_datetime(timestamp_us)
  ...
  describe "get_timeout/3" do
    test "returns default timeout from state" do
      ...
      timeout = RPC.get_timeout(method, nil, state)
    ...
    test "uses method-specific timeouts" do
      ...
      assert RPC.get_timeout("public/auth", nil, state) == 30_000
      assert RPC.get_timeout("private/logout", nil, state) == 5000
      assert RPC.get_timeout("public/test", nil, state) == 2000
      assert RPC.get_timeout("public/get_time", nil, state) == 5000
    end
    ...
    test "uses timeout from options when provided" do
      ...
      assert RPC.get_timeout("public/auth", options, state) == 3000
    end

    test "uses default 10000ms when state has no request_timeout" do
      state = %{}

      assert RPC.get_timeout("some/unknown/method", nil, state) == 10_000
    end
  end
  ...
  describe "generate_timeout_reference/1" do
    test "returns a reference that will trigger a timeout message" do
      ...
      ref = RPC.generate_timeout_reference(request_id)
