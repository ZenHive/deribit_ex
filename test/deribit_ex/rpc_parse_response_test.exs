defmodule DeribitEx.RPCParseResponseTest do
  use ExUnit.Case
  ...
  describe "parse_response/1" do
    test "returns {:ok, result} when result field present" do
      ...
      assert RPC.parse_response(resp) == {:ok, "success"}
    end
    ...
    test "returns {:ok, result} with complex result data structure" do
      ...
      assert RPC.parse_response(resp) == {:ok, result}
    end
    ...
    test "returns {:error, error_map} when error field present" do
      ...
      assert RPC.parse_response(resp) == {:error, error}
    end

    test "returns {:error, {:invalid_response, _}} on unexpected structures" do
      # Empty map
      assert match?({:error, {:invalid_response, %{}}}, RPC.parse_response(%{}))

      # Map with irrelevant keys
      bad_resp = %{"foo" => "bar", "baz" => 123}
      assert match?({:error, {:invalid_response, ^bad_resp}}, RPC.parse_response(bad_resp))

      # Nil response
      assert match?({:error, {:invalid_response, nil}}, RPC.parse_response(nil))
    end
  end
  ...
  describe "classify_error/1" do
    test "classifies auth errors correctly" do
      ...
      assert RPC.classify_error(error) == {:auth, :token_expired, "Token expired"}

      error = %{"code" => 13_004, "message" => "Custom message"}
      assert RPC.classify_error(error) == {:auth, :not_authorized, "Custom message"}
    end

    test "classifies rate limit errors correctly" do
      error = %{"code" => 10_429, "message" => "Too many requests"}

      assert RPC.classify_error(error) ==
    ...
    test "classifies validation errors correctly" do
      ...
      assert RPC.classify_error(error) == {:validation, :invalid_params, "Invalid params"}
    end

    test "classifies order errors correctly" do
      error = %{"code" => 10_009, "message" => "Not enough funds"}
      assert RPC.classify_error(error) == {:order, :insufficient_funds, "Not enough funds"}
    end

    test "returns unknown classification for unknown error codes" do
      error = %{"code" => 99_999, "message" => "Unknown error"}
      assert RPC.classify_error(error) == {:unknown, :unknown_code, "Unknown error"}
    end
  ...
  describe "needs_reauth?/1" do
    test "returns true for authentication errors" do
      assert RPC.needs_reauth?(%{"code" => 13_004}) == true
      assert RPC.needs_reauth?(%{"code" => 13_009}) == true
      assert RPC.needs_reauth?(%{"code" => 13_010}) == true
      assert RPC.needs_reauth?(%{"code" => 13_011}) == true
    end

    test "returns false for non-authentication errors" do
      assert RPC.needs_reauth?(%{"code" => 10_001}) == false
      assert RPC.needs_reauth?(%{"code" => 10_429}) == false
      assert RPC.needs_reauth?(%{"code" => 11_003}) == false
      assert RPC.needs_reauth?(%{"code" => 99_999}) == false
    end

    test "returns false for invalid error format" do
      assert RPC.needs_reauth?(%{}) == false
      assert RPC.needs_reauth?(nil) == false
    end
  end
  ...
  describe "extract_metadata/1" do
    test "extracts subscription channel metadata" do
      ...
      assert RPC.extract_metadata(notification) == expected
    end
    ...
    test "returns empty map for responses with no metadata" do
      ...
      assert RPC.extract_metadata(response) == %{}
    end
  end
end
